...
function redentSnapshot(input: string) {
  ...
  for (const line of lines) {
    ...
    const spacesInLine = (line.match(/^[+-](\s+)/g) || []).pop()!.length - 1
    minSpaces = Math.min(minSpaces, spacesInLine)
  }

  const replacer = new RegExp(`^([+-])\\s{${minSpaces}}(.*)`, 'g')
  ...
  const uniqueSnapshots = snapshots
    ...
    .map((snapshot, i, all) => ({
      ...
      relativeToPreviousSnapshot:
        i === 0
          ? 0
          : Number((snapshot.recordedAt - all[i - 1].recordedAt) / BigInt(1e6)),
    }))
