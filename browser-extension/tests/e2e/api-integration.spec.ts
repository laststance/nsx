...
test.describe('Extension API Integration Tests', () => {
  ...
  test('handles 500 error response correctly', async ({
    context,
    extensionId,
    page,
  }) => {
  ...
  test('backend server is accessible', async ({ request }) => {
    ...
    const response = await request.get('http://localhost:4000/api/user_count')
  ...
  test('multiple concurrent API calls', async ({
    ...
    await Promise.all([checkbox1.check(), checkbox2.check()])
