...
import {
  ...
  TOKEN_NOT_SAVED_MESSAGE,
} from './constants'
import { maskPatToken } from './utils/maskPatToken'

/** What the line under the token field says after Connect; '' shows no line. */
type ConnectErrorMessage =
  '' | typeof INVALID_TOKEN_MESSAGE | typeof TOKEN_NOT_SAVED_MESSAGE
...
const App: FC = () => {
  ...
  const [connectErrorMessage, setConnectErrorMessage] =
    useState<ConnectErrorMessage>('')
   ...
   * @returns Nothing; the hook stores the token and the card switches to the connected state, or the line under the field says why nothing was stored.
   * @example
   * <form onSubmit={onConnectSubmit}>
   */
  const onConnectSubmit = (event: FormEvent<HTMLFormElement>): void => {
    event.preventDefault()
    void connect(pastedToken)
      .then((isConnected) => {
        // Only a stored token clears the field; a refused value stays, flagged, until it is edited.
        if (isConnected) setPastedToken('')
        setConnectErrorMessage(isConnected ? '' : INVALID_TOKEN_MESSAGE)
      })
      // chrome.storage refused the write: the token stays in the field, so pressing Connect again retries.
      .catch(() => setConnectErrorMessage(TOKEN_NOT_SAVED_MESSAGE))
  }
              ...
              <form className="options-form" onSubmit={onConnectSubmit}>
                ...
                <div className="options-row">
                  <input
                    ...
                    autoComplete="off"
                    aria-invalid={connectErrorMessage === INVALID_TOKEN_MESSAGE}
                    aria-describedby={
                      connectErrorMessage ? 'pat-input-error' : undefined
                    }
                    onChange={(event): void => {
